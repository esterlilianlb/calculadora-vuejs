    ...
    <div class="calculator">
        ...
        <div class="scroll">
            <div class="student-info" v-bind:key="index" v-for="(student, index) in students">
                <p><strong>{{student.name}}</strong> precisa tirar, no mínimo, <strong>{{student.nota}}</strong></p>
                <span><a href="#" v-on:click.prevent="removeStudent(index)">OK</a></span>
            </div>
        </div>
...
export default {
    ...
    methods: {
        calculate() {
            if(this.name.trim() === '' || this.nota1.trim() === '' || this.nota2.trim() === '') {
                return;
            }
            const nota3 = (6.2 - (parseFloat(this.nota1*0.25) + parseFloat(this.nota2*0.25)))/0.5;
